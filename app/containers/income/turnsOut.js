...
class turnsOut extends Component {
    ...
    handleSubmit = () => {
        ...
        this.props.dispatch({
            type: 'income/interestChangeInOut',
            payload: {
                accountId: this.props.userInfo.accountCommonDTO.id,
                amount: strip(num),
            },
            callback: () => {
                Toast.info('转出成功', 1.5, null, true)
                setTimeout(() =>{
                    this.props.dispatch({
                ...
                <View style={styles.container}>
                    <View style={{margin: 5, marginTop: 10}}>
                        <List>
                            ...
                            <Item extra={`${this.props.income.currencyEarnings.totalAmount / 1000000}`}>我的余数</Item>
                            <InputItem
                                editable={true}
                                ...
                                type='number'
                                onChangeText={ (text) => {
                                    var energyNum = this.props.income.currencyEarnings.totalAmount  / 1000000
                                    this.setState({
                                        amount: text
                                    })
                                    let value = text
                    ...
                    <View style={{margin: 20}}>
                        <Button type="primary" disabled={this.state.disabled} style={styles.class_but} onClick={this.handleSubmit}>确定转出</Button>
                    </View>
                </View>
            </ScrollView>
        )
...
export default connect(({app, user, income}) => ({...app, user, income}))(turnsOut)